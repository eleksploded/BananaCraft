...
{
    protected static final IBlockState field_185387_y = Blocks.waterlily.getDefaultState();

    protected BiomeGenSwamp(BiomeGenBase.BiomeProperties properties)
    {
        super(properties);
            ...
            for (int k = 255; k >= 0; --k)
            {
                if (chunkPrimerIn.getBlockState(j, k, i).getMaterial() != Material.air)
                {
                    if (k == 62 && chunkPrimerIn.getBlockState(j, k, i).getBlock() != Blocks.water)
                    {
                        ...
                        if (d0 < 0.12D)
                        {
                            chunkPrimerIn.setBlockState(j, k + 1, i, field_185387_y);
                        }
                    }
