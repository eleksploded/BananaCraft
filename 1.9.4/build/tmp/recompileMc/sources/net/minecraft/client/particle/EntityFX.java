...
public class EntityFX
{
    ...
    public EntityFX(World worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn, double ySpeedIn, double zSpeedIn)
    {
        ...
        this.xSpeed = xSpeedIn + (Math.random() * 2.0D - 1.0D) * 0.4000000059604645D;
        this.ySpeed = ySpeedIn + (Math.random() * 2.0D - 1.0D) * 0.4000000059604645D;
        this.zSpeed = zSpeedIn + (Math.random() * 2.0D - 1.0D) * 0.4000000059604645D;
        float f = (float)(Math.random() + Math.random() + 1.0D) * 0.15F;
        float f1 = MathHelper.sqrt_double(this.xSpeed * this.xSpeed + this.ySpeed * this.ySpeed + this.zSpeed * this.zSpeed);
        this.xSpeed = this.xSpeed / (double)f1 * (double)f * 0.4000000059604645D;
        this.ySpeed = this.ySpeed / (double)f1 * (double)f * 0.4000000059604645D + 0.10000000149011612D;
        this.zSpeed = this.zSpeed / (double)f1 * (double)f * 0.4000000059604645D;
    }

    public EntityFX multiplyVelocity(float multiplier)
    {
        this.xSpeed *= (double)multiplier;
        this.ySpeed = (this.ySpeed - 0.10000000149011612D) * (double)multiplier + 0.10000000149011612D;
        this.zSpeed *= (double)multiplier;
        return this;
    }
    ...
    public void onUpdate()
    {
        ...
        this.ySpeed -= 0.04D * (double)this.particleGravity;
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        double d1 = y;
        double d2 = z;
        List<AxisAlignedBB> list = this.worldObj.getCubes((Entity)null, this.getEntityBoundingBox().addCoord(x, y, z));
