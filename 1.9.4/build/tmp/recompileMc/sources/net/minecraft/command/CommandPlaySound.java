...
public class CommandPlaySound extends CommandBase
{
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        ...
        else
        {
            int i = 0;
            String s = args[i++];
            SoundCategory soundcategory = SoundCategory.getByName(args[i++]);
            ...
            entityplayermp.playerNetServerHandler.sendPacket(new SPacketCustomSound(s, soundcategory, d0, d1, d2, (float)d3, (float)d4));
    ...
    {
        return args.length == 1 ? getListOfStringsMatchingLastWord(args, SoundEvent.soundEventRegistry.getKeys()) : (args.length == 2 ? getListOfStringsMatchingLastWord(args, SoundCategory.getSoundCategoryNames()) : (args.length == 3 ? getListOfStringsMatchingLastWord(args, server.getAllUsernames()) : (args.length > 3 && args.length <= 6 ? getTabCompletionCoordinate(args, 2, pos) : Collections.<String>emptyList())));
    }
